...
def process_code_execution(
    ...
    if code_blocks:
        # Execute only the first block by default. This keeps the loop interactive
        # and avoids speculative fallback blocks running before the model sees outputs.
        executed_blocks = code_blocks[:max_code_blocks]
        for code in executed_blocks:
            ...
            messages.append(
                {
                    "role": "user",
                    "content": (
                        f"Only the first {max_code_blocks} REPL block was executed from your previous message. "
                        "Use the next turn to run follow-up code after observing the output."
                    ),
                }
            )
...
def convert_context_for_repl(context):
    """
    Convert REPL context to either JSON-compatible data, a raw string,
    or a direct Python object (e.g., Monocontext).
    """
    context_obj = None
    ...
    return context_data, context_str, context_obj
