...
class REPLEnv:
    def __init__(
        ...
        context_obj: Optional[object] = None,
        setup_code: str = None,
    ):
        ...
        self.load_context(context_json, context_str, context_obj)
        
        def llm_query(prompt: str) -> str:
            """Query the LLM with the given prompt."""
            print("[REPLEnv] llm_query: sending prompt to sub model")
            start_time = time.time()
            response = self.sub_rlm.completion(prompt)
            elapsed = time.time() - start_time
            print(f"[REPLEnv] llm_query: response received in {elapsed:.2f}s")
            return response
    ...
    def load_context(
        ...
        if context_obj is not None:
            # Directly inject object into REPL locals.
            self.locals["context"] = context_obj
    ...
    def code_execution(self, code) -> REPLResult:
        ...
        """
        print(f"[REPLEnv] code_execution: start (chars={len(code)})")
        start_time = time.time()
        ...
        print(
            f"[REPLEnv] code_execution: done in {execution_time:.2f}s "
            f"(stdout_chars={len(stdout_content)}, stderr_chars={len(stderr_content)})"
        )
